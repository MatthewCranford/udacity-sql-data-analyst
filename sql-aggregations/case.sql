...
SELECT a.name, DATE_PART('year',o.occurred_at) year_of_sales, SUM(o.total_amt_usd) total_sales,
	CASE WHEN SUM(o.total_amt_usd) > 200000 THEN 'top'
    WHEN SUM(o.total_amt_usd) > 100000 THEN 'middle'
    ELSE 'low' END AS rank
FROM accounts a
...
SELECT s.name, COUNT(o.id), CASE WHEN COUNT(o.id) > 200 THEN 'top' ELSE 'not' END AS status
FROM accounts a
JOIN orders o 
ON a.id = o.account_id
JOIN sales_reps s
...
SELECT s.name, COUNT(o.id) num_sales, SUM(o.total_amt_usd) total_sales, 
CASE 
